...
IF (CHARINDEX ('Azure', @@VERSION) = 0) BEGIN
  PRINT ''
  PRINT '**** NOTE ****'
...
END
ELSE BEGIN
  PRINT 'This script captures snapshots of various DMVs from your database every 10 seconds.'
  PRINT 'It is to be executed for the duration of the issue ( typically for not more than 30 minutes) '
  PRINT 'The output is written to file named [SERVER]_SQL_Azure_Perf_Stats.OUT'
  PRINT ''
END
GO
...
IF (CHARINDEX ('10.0', @@VERSION) = 0) BEGIN
 ...
 -- PRINT '**** This script is for SQL Server Denali.  Errors are expected when run on earlier versions.'
  PRINT '**** This script is for SQL Azure. Errors are expected when run on earlier versions.'
  PRINT '**************'
  PRINT ''
END
GO
...
DECLARE @sp_perf_stats_azure NVARCHAR(MAX)
DECLARE @ParmDefinition NVARCHAR(500);
SET @ParmDefinition = N'@appname sysname, @runtime datetime';
SET @sp_perf_stats_azure = N'SET NOCOUNT ON
...
DECLARE @sp_perf_stats_azure_infrequent11 NVARCHAR(MAX)
DECLARE @ParmDefinition2 NVARCHAR(500);
SET @ParmDefinition2 = N' @runtime datetime, @firstrun int';
SET @sp_perf_stats_azure_infrequent11=  N'SET NOCOUNT ON
  DECLARE @queryduration int
  DECLARE @querystarttime datetime
  DECLARE @qrydurationwarnthreshold int
  DECLARE @cpu_time_start bigint, @elapsed_time_start bigint
...
ELSE BEGIN
  ...
  BEGIN
    ...
    EXECUTE sp_executesql @sp_perf_stats_azure, @ParmDefinition,@appname = '%appname%', @runtime = @runtime;


	/*Added 01/30 -Rohitna*/
	-- Collect sp_perf_stats_infrequent11 every minute
    IF @i = 0
      EXECUTE sp_executesql @sp_perf_stats_azure_infrequent11,@ParmDefinition2, @runtime = @runtime, @firstrun = 1
    ELSE IF @i % 5 = 0
      EXECUTE sp_executesql @sp_perf_stats_azure_infrequent11,@ParmDefinition2, @runtime = @runtime, @firstrun = 0
